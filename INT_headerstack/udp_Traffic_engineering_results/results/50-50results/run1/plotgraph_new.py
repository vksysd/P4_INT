...
df3x = pd.DataFrame(
    {'Time in seconds': time_at_s3x,
     'Queue depth at S3 without INT module': qdepth_at_s3x,
     })
...
df3 = pd.DataFrame(
    {'Time in seconds': time_at_s3,
     'Queue depth at S3 with INT module': qdepth_at_s3,
     })
ax = df3.plot(x='Time in seconds',y='Queue depth at S3 with INT module')
constant = np.ones(shape=(471, 1)) * 150
constantDF = pd.DataFrame(constant, columns=['Rate Limit'])
constantDF.plot(ax = ax)
df3x.plot(x='Time in seconds',y='Queue depth at S3 without INT module', ax = ax)
#plt.show()
# df.plot(x="time",y=qdepth_at_s3,kind="bar")
plt.ylabel("Queue Depth")
plt.title("Queue Depth v/s Time at Switch 3 for 50:50")
