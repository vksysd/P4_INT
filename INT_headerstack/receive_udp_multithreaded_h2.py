            ...
            if i == 2 :
                ...
                if print_qdepth_flag:
                    ...
                    if (int(p2.qdepth2) > QDEPTH_THRESHHOLD) & update_flag == 1:
                        ...
                        # get_entry_num = str("echo table_dump_entry_from_key ipv4_lpm "+str(pkt[IP].src)+" "+str(pkt[IP].dst)+"/32 "+str(pkt[UDP].sport)+" "+str(pkt[UDP].dport)+" | simple_switch_CLI --thrift-port 9091 | grep "+str("Dumping entry")+" | awk '{print $4}'")
                        # print("get_entry_num = ",get_entry_num)

                        # z = commands.getstatusoutput(get_entry_num)
                        # print("z = ",z)
                        # entry_num = z.split(",")[1].split("x")[1]
                        # entry_num = 2*(port_identified-7000)+1
                        # last_modified = i
                        print("entry num = ", entry_num)
                        # next_port = 3
                        next_port_list.append(next_port)
                        command_to_run = "echo table_modify ipv4_lpm ipv4_forward "+str(entry_num)+" "+str(pkt[Ethernet].dst)+" "+str(next_port)+" | simple_switch_CLI --thrift-port 9091"
...
def main():
    ...
    sys.stdout.flush()
    #print ("before sniff")
    sniff(filter="udp and host 10.0.1.2 and dst port 9090", iface = iface,
